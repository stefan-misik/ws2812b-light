...
class ColorGridView:
    ...
    def __init__(self, root, model: ColorGridModel):
        ...
        ttk.Entry(top_frame, width=10, textvariable=self._frame_counter, state='readonly', justify="right") \
            .pack(side=tk.RIGHT)
        ttk.Label(top_frame, text="Counter:").pack(side=tk.RIGHT)
        ...
        # Buttons
        self.apply_btn = ttk.Button(mid_frame, text="⏹", command=self._on_stop)
        self.apply_btn.pack(side=tk.LEFT, expand=True, anchor=tk.E, padx=5)

        self.reset_btn = ttk.Button(mid_frame, text="▶", command=self._on_play)
        self.reset_btn.pack(side=tk.LEFT, padx=5)

        self.random_btn = ttk.Button(mid_frame, text="⏸", command=self._on_pause)
        self.random_btn.pack(side=tk.LEFT, expand=True, anchor=tk.W, padx=5)

        # Canvas for circles
        self._canvas = tk.Canvas(root, width=400, height=400, bg="white", borderwidth=4, relief="groove")
