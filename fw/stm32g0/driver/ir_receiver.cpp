...
{
public:
    using Packet = IrReceiver::Packet;
     ...
     * @return Is done
     */
    bool processSymbol(Symbol symbol)
    {
        if (-1 == pos_)
        {
...
bool IrReceiver::initialize(TimerId tim_id, std::uint8_t dma_channel_id)
{
...
IrReceiver::Packet IrReceiver::read()
{
    auto & priv = p();

    Packet received = Packet::Invalid();
    while (true)
    {
        ...
        const std::uint8_t * const end = buffer + length;
        for (; pos != end; pos += 2)
        {
            ...
            {
                received = priv.packet_state.checkAndGet();
                pos += 2;
                break;
            }
        }
        ...
        if (received.type() != Packet::Type::INVALID)
            break;
    };
