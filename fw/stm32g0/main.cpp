...
driver::IrReceiver::Packet ir_received;
std::uint32_t ir_repeat = 0;
...
int main()
{
    ...
    keypad.initialize({ {driver::GpioId::GPIO_A, 15}, {driver::GpioId::GPIO_B, 3}, {driver::GpioId::GPIO_B, 7},
        {driver::GpioId::GPIO_B, 4}, {driver::GpioId::GPIO_B, 5}, {driver::GpioId::GPIO_B, 6}, });
    ...
    while (true)
    {
        ...
        if (led_update_timer.shouldRun(current_time, 8))
        {
            ...
            {
                const auto rcv = ir_receiver.read();
                if (rcv.type() == driver::IrReceiver::Packet::Type::NORMAL)
                {
                    ir_received = rcv;
                    ir_repeat = 0;
                }
                else if (rcv.type() == driver::IrReceiver::Packet::Type::REPEAT)
                {
                    ir_repeat += 1;
                }
            }
